<template>
	<div class="echartsDiv">
		<div class="data" style="margin-left:20px;margin-bottom:20px" v-for="(item, i) in viewData" :key="i">
			...
			<p class="name">
				<span class="value">{{ viewData[i].data[viewData[i].data.length - 1] }}秒 </span>
				<span class="num">环比 {{ monHb[i].hb }}</span>
			</p>

			<div class="charts_bar_horizontal" :id="'echarts' + i"></div>
			<div class="line"></div>
				...
				for (let j = 0; j < data.city[0].city_hb.length; j++) {
					if (data.city[0].city[i].cityName == data.city[0].city_hb[j].cityName) {
						tableData1.push({
							...
							data: data.city[0].city[i].yytblsc,
							hb: data.city[0].city_hb[j].yytblsc,
						});
					}
				}
			}
