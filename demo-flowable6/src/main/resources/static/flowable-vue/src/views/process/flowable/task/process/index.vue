  ...
  <div class="app-container">
    ...
    <el-dialog :title="title" :visible.sync="open" width="60%" append-to-body>
      <el-table v-loading="processLoading" fit :data="definitionList" border>
        ...
        <el-table-column label="业务表单" align="center">
          <template slot-scope="scope">
            <span v-if="scope.row.formId">{{ scope.row.formName }}</span>
...
<script>
import {addDeployment, exportDeployment, getDeployment, updateDeployment} from "@/api/flowable/finished.js";
import {deleteInstance, myProcessList, stopProcess} from "@/api/flowable/process.js";
  ...
  methods: {
    ...
    getList() {
      ...
      myProcessList(this.queryParams).then(res => {
        this.myProcessList = res.data.data.list;
    ...
    listDefinition() {
      listDefinition(this.queryParams).then(res => {
        this.definitionList = res.data.data.list;
        this.processTotal = res.data.data.total;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateDeployment(this.form).then(res => {
              this.$message.success("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addDeployment(this.form).then(res => {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      var ids = row.procInsId || this.ids;
      const querys = {
        "ids": ids
      }
      ...
      this.$confirm('是否确认删除流程实例', "警告", {
        ...
        deleteInstance(querys).then(res => {
