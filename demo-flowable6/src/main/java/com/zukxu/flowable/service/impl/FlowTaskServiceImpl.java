...
public class FlowTaskServiceImpl extends FlowFactory implements FlowTaskService {
    ...
    public R<?> revokeProcess(FlowTaskVo flowTaskVo) {
        ...
        SysUser loginUser = InitUtils.getUserByUserId("1");
        List<HistoricTaskInstance> htiList = historyService.createHistoricTaskInstanceQuery().processInstanceId(task.getProcessInstanceId()).orderByTaskCreateTime().asc().list();
        ...
        List<HistoricActivityInstance> haiList = historyService.createHistoricActivityInstanceQuery().executionId(myTask.getExecutionId()).finished().list();
