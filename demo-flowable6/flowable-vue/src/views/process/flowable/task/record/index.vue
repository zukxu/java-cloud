...
import {definitionStart, getFlowViewer, getProcessVariables, readXml} from "@/api/flowable/definition.js";
import {complete, delegate, getNextFlowNode, rejectTask, returnList, returnTask} from "@/api/flowable/todo.js";
import flow from '@/views/process/flowable/task/record/flow.vue'
  ...
  methods: {
    ...
    getFlowRecordList(procInsId, deployId) {
      ...
      flowRecord(params).then(res => {
        this.flowRecordList = res.data.data.flowList;
        // 流程过程中不存在初始化表单 直接读取的流程变量中存储的表单值

        if (res.data.data.formData) {
          this.formConf = res.data.data.formData;
    ...
    processVariables(taskId) {
      if (taskId) {
        ...
        getProcessVariables(taskId).then(res => {
          this.variablesData = res.data.data.variables;
        ...
        if (data) {
          if (data.type === 'assignee') {
            ...
            data.roleList.forEach(role => {
              role.userId = role.id;
              role.loginName = role.roleName;
            })
            this.userDataList = data.roleList;
    ...
    handleReturn() {
      ...
      returnList(this.taskForm).then(res => {
        this.returnTaskList = res.data.data;
