...
public class UserController {
    ...
    public R register(@Valid @RequestBody User user, BindingResult errors) {
        ...
        if (errors.hasErrors()) {
            ...
            return R.fail(RStatus.PARAM_ERROR.getMsg());
        }
        //输入入库
    ...
    @GetMapping("/getById2")
    @Validated
    public R getById2(@NotBlank(message = "ID不能为空") @RequestParam(name = "id") String id,
                      @Range(min = 0, max = 120, message = "年龄最小为0，最大为120") @RequestParam(name = "age") Integer age
                     ) {
