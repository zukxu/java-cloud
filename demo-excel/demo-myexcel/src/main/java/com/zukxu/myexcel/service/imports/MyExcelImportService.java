...
public class MyExcelImportService {
    ...
    public void defaultImport(MultipartFile file) {
        ...
        DefaultExcelReader.of(ArtCrowd.class).sheet(0) // 0代表第一个，如果为0，可省略该操作，也可sheet("名称")读取
                          .rowFilter(row -> row.getRowNum() > 0) // 如无需过滤，可省略该操作，0代表第一行
                          .beanFilter(ArtCrowd::isDance) // bean过滤
                          .readThen(file.getInputStream(), artCrowd -> {System.out.println(artCrowd.getName());});// 可接收inputStream
    ...
    public void saxImport(MultipartFile file) {
        ...
        List<ArtCrowd> result = SaxExcelReader.of(ArtCrowd.class).sheet(0) // 0代表第一个，如果为0，可省略该操作，也可sheet("名称")读取，.csv文件无效
                                              .rowFilter(row -> row.getRowNum() > 0) // 如无需过滤，可省略该操作，0代表第一行
    ...
    public void columnReadImport(MultipartFile file, HttpServletResponse response) {
        ...
        // 读取为字符串
        List<String> strings = ColumnSaxExcelReader.columnNum(0).rowFilter(row -> row.getRowNum() > 1).readAsString(excelFile);

        // 读取为Integer
        List<Integer> integers = ColumnSaxExcelReader.columnNum(1).rowFilter(row -> row.getRowNum() > 1).readAsInteger(excelFile);

        // 读取为Long
        List<Long> longs = ColumnSaxExcelReader.columnNum(2).rowFilter(row -> row.getRowNum() > 1).readAsLong(excelFile);
    }
